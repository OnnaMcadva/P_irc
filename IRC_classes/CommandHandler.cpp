...
/* Функция `processCommand` обрабатывает команды от клиента IRC-сервера.  

1. **Проверяет, есть ли клиент в базе**  
   - Если нет, удаляет его.  

2. **Проверяет ввод пароля:**  
   - Если пароль не введен, проверяет его.  
   - Если пароль правильный → авторизует клиента.  
   - Если неправильный → уменьшает количество попыток, при исчерпании удаляет клиента.  

3. **Обрабатывает команды после авторизации:**  
   - `QUIT` — отключает клиента.  
   - `NICK` — устанавливает никнейм, проверяет его доступность.  
   - `USER` — задает имя пользователя и реальное имя.  
   - `JOIN` — присоединяет клиента к каналу или создает новый.  
   - `PRIVMSG` — отправляет личное сообщение или сообщение в канал.  
   - **Любая другая команда** — возвращает ошибку "Unknown command".  

В итоге, функция управляет регистрацией клиента, обработкой его команд и взаимодействием с сервером. */
