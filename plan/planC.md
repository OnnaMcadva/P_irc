...
#### Что сделано правильно:
1. **Обработка базовых команд**: `PASS`, `NICK`, `USER`, `JOIN`, `PRIVMSG`, `QUIT` реализованы и работают в целом корректно.
...
2. **QUIT**: Клиент удаляется, но не отправляется сообщение другим участникам канала о его уходе (например, `:nick QUIT :Goodbye`).
...
2. **Очистка каналов**: При отключении клиента удалять его из всех каналов и уведомляй участников (`PART` или `QUIT`).
...
### Класс `Config`
#### Что сделано правильно:
1. **Простота**: Хранит порт и пароль, как требуется.

#### Что нужно исправить:
1. **Валидация**: Нет проверки длины пароля или его символов — это может быть уязвимостью.

#### Что нужно добавить:
1. **Конфигурационный файл** (опционально): Задание разрешает его использование, можно добавить парсинг файла для гибкости.

#### Рекомендации по улучшению:
1. **Константы**: Вынести минимальный/максимальный порт (1–65535) в константы.
2. **Безопасность**: Запретить пустой пароль или слишком короткий (например, < 4 символов).
